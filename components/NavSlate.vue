<script setup>
	...
	const openMenu = () => {
		gsap.registerPlugin(CSSPlugin);
		gsap.fromTo(".menu", { autoAlpha: 1, xPercent: -100 }, { xPercent: 0, duration: 0.8, ease: "expo.inOut" });
	};
	const closeMenu = () => {
		gsap.to(".menu", {
			xPercent: 200,
...
<template>
	<div class="fixed z-40 h-screen w-full bg-amber-100 flex flex-col place-content-center invisible menu font-NeueMontrealThin">
		...
		<div class="flex mx-auto">
			<ul class="text-slate-700 text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl">
	...
	<div class="fixed w-full flex flex-row place-content-between items-center z-30 font-NeueMontrealThin">
		<MenuIcon class="fill-slate-700 w-16 sm:w-18 md:w-20" @click="openMenu" />
		<span class="m-1"
			><NuxtLink to="/booking">
				<button class="border-2 border-slate-700 rounded-2xl transition delay-50 ease-in-out hover:border-brand-yellow duration:300">
					<p
						class="text-slate-700 p-1 sm:p-2 text-md sm:text-xl md:text-2xl transition delay-50 ease-in-out hover:text-brand-yellow duration:200"
					>
						Book now
					</p>
				</button>
